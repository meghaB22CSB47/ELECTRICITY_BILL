...
// Function to get report data based on filters for specific user
function getBillReport($year = null, $month = null) {
    global $conn;
    
    // Always filter by the logged-in user's ID
    $userId = $_SESSION['Userid'];
    
    $sql = "SELECT * FROM bill WHERE userId = '" . $conn->real_escape_string($userId) . "'";
...
// Get report data for the logged-in user
$reportData = getBillReport($year, $month);
...
<head>
    ...
    <title>My Electricity Bill Report</title>
...
<body>
    <div class="container">
        <h1>My Electricity Bill Report</h1>
